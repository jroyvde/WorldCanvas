...
class Entity {
    constructor(spawnX, spawnY) {
        ...
        entitiesOnCanvas.push(this)                     // Add object to the entitiesOnCanvas array
        this.entityIndex = entitiesOnCanvas.length - 1  // Remember unique entityIndex

        this.sprite.id(this.entityIndex.toString())     // Use the sprite's ID to remember its parent's Index
        this.variant = 0                                // Variant ID
    ...
    clearColor() {
        if (![...paintedImages.values()].includes(this.sprite.image())) return  // If the entity is not painted, do nothing
        ...
        if (unpaintedImage) this.sprite.image(unpaintedImage)
...
class Being extends Entity {
    constructor(spawnX, spawnY) {
        ...
        this.age = 0        // Age: Internal age. Old from 80, dead at 100. Animals should age faster than people. (Unused)
        this.love = 0       // Love should influence whether beings will gravitate towards others of the same type. Should be between -1 and 1. 0 = neutral. (Unused)
    ...
    // Roam randomly and continuously around the world
    roam() {
...
class Dog extends Being {
    constructor(spawnX, spawnY) {
        ...
        this.excitement = 0     // Excitement makes the doggo very fast. Default = 0, Max = 1 (Unused)
...
class Brush extends Inanimate {
    ...
    // Change animation and play sound when hitting the ground
    splatOnGround() {
        this.sprite.animation('landed')
...
class Foliage extends Inanimate {
    constructor(spawnX, spawnY) {
        ...
        this.grabbable = false          // Not grabbable
        this.mappedTool = foliageTool   // Set Mapped Tool
    ...
    // Function to increase by one growth stage
    grow() {
        if (this.growthStage >= 2) return
    ...
    // Function to reverse one growth stage
    prune() {
    ...
    // Function to change the foliage's climate type
    changeClimateType(input) {
        const newClimateType = parseClimate(input, this.climateType)

        if (newClimateType === this.climateType) return
...
class ThoughtBubble extends Inanimate {
    constructor(spawnX, spawnY) {
        ...
        this.grabbable = false      // Not grabbable
        this.mappedTool = timeTool  // Set Mapped Tool
